...
const MapSVG = ({ 
  ...
  // Web-specific state (disabled for mobile)
...
// Handler for when a start point is clicked
  ...
  // Drag handlers (web only - disabled for mobile)
  ...
  const handleStartPointHover = (isHovering) => {
    // Mobile doesn't use hover - kept for reference only
  };
  ...
  // Web-only function - kept for reference but not used on mobile
  // const findNearestNode = (clickX, clickY) => {
  //   ... finds nearest node from click coordinates ...
  // };

  // Web-only map click handler (disabled for mobile)
  // const handleMapClick = async (e) => {
  //   if (!isLocationToolActive) return;
  //   ... web-specific click code removed for mobile compatibility ...
  // };

  // Add CustomStartPoint component (not used on mobile yet)
  // const CustomStartPoint = ({ nodeId }) => {
  //   if (!nodeId || !nodes[nodeId]) return null;
  //   ... mobile version can be implemented later ...
  // };

  // Update return JSX to render SVG only (mobile compatible)
  return (
